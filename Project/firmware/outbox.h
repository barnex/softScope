...
typedef struct {
	...
	uint32_t trigLev;
	uint32_t timeBase;
	uint32_t padding[HEADER_WORDS-9]; // unused space, needed for correct total size, should be HEADER_WORDS minus number of words in the struct!
} header_t;

header_t *hdr;     // header written to software, first part of usartBuf
